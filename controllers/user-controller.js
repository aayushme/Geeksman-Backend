...
const signuphandler = async (req, res, next) => {
  ...
  if(pendinguser){
    const error=new HttpError('This email address is already registered,please verify',422)
    return next(error)
  }
  let hashedpassword;
  try {
...
const createuserbyadmin = async (req, res, next) => {
  ...
  if (existinguser) {
    const error = new HttpError('User already registered,Cannnot create new', 422)
    return next(error)
  }
  if (pendinguser) {
